...
export default function PaginaBusca() {
   ...
   const toggleSelection = <T,>(item: T, setState: React.Dispatch<React.SetStateAction<T[]>>) => {
      setState((prev) => (prev.includes(item) ? prev.filter((value) => value !== item) : [...prev, item]));
   };

   const filtros = useMemo<BuscaFiltros>(
      () => ({
         termo: searchTerm.trim() || undefined,
         marcas: selectedBrands.length > 0 ? selectedBrands : undefined,
      ...
      [searchTerm, selectedBrands, priceRange, apenasOferta, ordenar, maxPreco]
   );
   ...
   const marcasDisponiveis = useMemo(() => {
      ...
      produtosApi.forEach((produto) => {
         const nome = produto.marca_nome?.trim();
         if (nome) {
            marcasSet.add(nome);
         }
      });

      return Array.from(marcasSet).sort((a, b) => a.localeCompare(b, "pt-BR"));
   }, [produtosApi]);
   ...
   const marcasFiltradas = useMemo(() => {
      ...
      return marcasDisponiveis.filter((brand) => brand.toLowerCase().includes(termo));
   }, [brandSearchTerm, marcasDisponiveis]);

   const produtosFiltrados = useMemo(() => {
      const marcasAplicadas = filtrosAplicados.marcas ?? [];
      const base = marcasAplicadas.length
         ? produtosApi.filter((produto) => produto.marca_nome && marcasAplicadas.includes(produto.marca_nome))
         : produtosApi;

      return base.map(mapParaResumo);
   }, [produtosApi, filtrosAplicados.marcas]);
               ...
               {marcasFiltradas.length === 0 ? (
                  ...
                  <ul className="mt-6 space-y-4" aria-label="Brand options">
                     {marcasFiltradas.map((brand) => (
                        <li key={brand}>
                           <label htmlFor={`brand-${brand.toLowerCase()}`} className="inline-flex items-center gap-2.5 group cursor-pointer">
                              <input
                                 type="checkbox"
                                 className="sr-only"
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
               <div>
                  ...
                  <p className="text-sm text-slate-600" aria-live="polite">
                     {(filtrosAplicados.marcas?.length ? produtosFiltrados.length : totalResultados)} resultado{(filtrosAplicados.marcas?.length ? produtosFiltrados.length : totalResultados) === 1 ? "" : "s"}
                  </p>
               </div>
